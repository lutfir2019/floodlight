...
public class LBPool {
	...
	protected final static short LEAST_CONNECTION = 4;
	...
	public String pickMember(IPClient client, HashMap<String,U64> membersBandwidth,HashMap<String,Short> membersWeight,HashMap<String, Integer> memberConnections,HashMap<String, Short>  memberStatus) {
		if(members.size() > 0){
			...
			//least connection	
			} else if (lbMethod == LEAST_CONNECTION && !memberConnections.isEmpty()) {
				return pickLeastConnectionMember(memberConnections);
			} else {
				if(LoadBalancer.isMonitoringEnabled && !monitors.isEmpty() && !memberStatus.isEmpty()){  // if health monitors active
	...
	public String pickLeastConnectionMember(HashMap<String, Integer> memberConnections) {
		if (!memberConnections.isEmpty()) {
			ArrayList<String> eligibleMembers = new ArrayList<>();

			// Find the member(s) with the minimum number of connections
			int minConnections = Collections.min(memberConnections.values());
			for (String memberId : memberConnections.keySet()) {
				if (memberConnections.get(memberId) == minConnections) {
